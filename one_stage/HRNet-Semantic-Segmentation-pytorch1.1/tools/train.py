...
from core.criterion import CrossEntropy, OhemCrossEntropy, cross_entropy_loss2d, error_loss
...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        help='experiments/cityscapes/seg_hrnet_w48_train_512x1024_sgd_lr1e-2_wd5e-4_bs_12_epoch484.yaml',
                        required=True,
                        type=str)
...
def main():
    ...
    model = FullModel(model, criterion, cross_entropy_loss2d, error_loss)
    ...
    if config.TRAIN.OPTIMIZER == 'sgd':
        optimizer = torch.optim.SGD([{'params': filter(lambda p: p.requires_grad, model.module.get_decoder_params()), 'lr': config.TRAIN.LR},
                                     {'params': filter(lambda p: p.requires_grad, model.module.get_backbone_params()), 'lr': config.TRAIN.LR / 10.0}],
                                lr=config.TRAIN.LR,
